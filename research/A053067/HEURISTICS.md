...
\[
\frac{1}{\log A(n)}
\left((1-\tfrac12)(1-\tfrac13)(1-\tfrac15)\right)^{-1}
=
\frac{3.75}{\log A(n)}.
\]
...
The last column is \(1-e^{-\lambda}\), with the preceding column used as
...
The terms become longer roughly like \(2n\log_{10}n\), so the individual prime
hazard falls approximately as \(1/(n\log n)\). The cumulative hazard therefore
grows only like \(\log\log n\).
