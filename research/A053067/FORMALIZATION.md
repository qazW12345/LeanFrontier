...
\[
L=L_n=\frac{n(n-1)}2+1,\qquad
U=U_n=\frac{n(n+1)}2=L_n+n-1.
\]
...
If an odd prime \(p\mid q-1\), prove

\[
2A(n)\equiv n(n^2+1)\pmod p.
\]

Then any proof of \(p\mid n(n^2+1)\) yields \(p\mid A(n)\), because \(p\) is
odd.

If \(p\mid q+1\), prove the parity split:

- even \(n\): \(2A(n)\equiv n\pmod p\);
- odd \(n\): \(2A(n)\equiv n^2+1\pmod p\).

For general \(p\nmid10(q-1)\), use the closed form together with a hypothesis
on \(q^n\bmod p\). This is the reusable theorem behind the periodic-family
search.
...
\[
10^{d-1}\le L_n\quad\text{and}\quad U_n<10^d.
\]

The triangular blocks partition the positive integers, so there is exactly one
mixed-width exceptional index for each power-of-ten boundary. That exceptional
sequence can be handled separately in a global argument.
...
- the single mixed-width exceptional index at each boundary.
